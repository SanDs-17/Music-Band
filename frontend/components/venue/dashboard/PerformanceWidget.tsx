      ...
      <CardContent className="p-4 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-3 bg-bg-elevated/20 border border-border/50 rounded-xl space-y-1.5">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-lg font-black text-white">{performance.booking_growth}%</span>
          ...
          <div className="p-3 bg-bg-elevated/20 border border-border/50 rounded-xl space-y-1.5">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-lg font-black text-white">{performance.revenue_growth}%</span>
              <span className="text-xs text-emerald-400 font-bold flex items-center">
                <ArrowUpRight className="h-3 w-3" />
                <span>MoM</span>
              </span>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-white uppercase tracking-wider">Monthly Occupancy Rates</span>
          </div>
            ...
            {performance.monthly_occupancy.map((item) => (
              <div key={item.month} className="space-y-1">
                <div className="flex justify-between text-[10px] font-semibold text-text-secondary">
                  ...
                  <span className="text-white">{item.occupancy}%</span>
                </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-white uppercase tracking-wider">Top Event Bookings</span>
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            {performance.top_event_types.map((type) => (
              <div key={type.name} className="p-2.5 bg-bg-elevated/30 border border-border/40 rounded-xl flex items-center justify-between">
                ...
                <span className="text-xs font-black text-white bg-primary/20 border border-primary/30 px-2 py-0.5 rounded-md">
                  {type.value}%
                </span>
              </div>
            ))}
