...
from app.api.band_common.storage import save_upload
from app.api.band_venues import service, crud
...
@router.get("/band/venues", response_model=schemas.BandPaginatedVenueList, tags=["Band Venues"])
def public_list(search: str | None = None,
                limit: int = Query(20, ge=1, le=100),
                offset: int = Query(0, ge=0),
                db: Session = Depends(get_db)):
    items, total = crud.list_filtered(db, search, "approved", limit, offset)
    return {"items": [crud.serialize(venue) for venue in items], "total": total}
