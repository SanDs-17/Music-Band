...
from app.api.band_bookings import crud
...
def admin_update_status(db: Session, admin_id: int, booking_id: int, new_status: str):
    ...
    if new_status == "accepted":
        _transition(db, booking, "accepted", "admin", "Booking accepted by admin")
    elif new_status == "rejected":
        _transition(db, booking, "rejected", "admin", "Booking rejected by admin")
    elif new_status == "cancelled":
        _transition(db, booking, "cancelled", "admin", "Booking cancelled by admin")
    elif new_status == "completed":
        if booking.status != "accepted":
            raise HTTPException(status_code=400, detail="Only accepted bookings can be completed.")
        _transition(db, booking, "completed", "admin", "Booking marked completed by admin")
    else:
...
def create_booking(db: Session, client_id: int, data):
    ...
    if data.venue_id:
        ...
        from app.api.band_venues.service import check_booking_conflict
    ...
    if data.artist_profile_id:
        ...
        from app.api.band_artists.service import check_availability_conflict
