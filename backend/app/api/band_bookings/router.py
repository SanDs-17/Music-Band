...
from app.api.band_bookings import service
...
@router.put("/band/bookings/admin/{booking_id}/status", tags=["Band Bookings Admin"])
def admin_update_booking_status(
    booking_id: int,
    payload: schemas.BandBookingStatusUpdate,
    db: Session = Depends(get_db),
    _admin: BandAccount = Depends(require_band_role("admin")),
):
    return service.admin_update_status(db, _admin.id, booking_id, payload.status)
