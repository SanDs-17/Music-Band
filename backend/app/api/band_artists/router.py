...
@router.get("/band/artists", response_model=schemas.BandPaginatedArtistList, tags=["Band Artists"])
def public_list(search: str | None = None,
                limit: int = Query(20, ge=1, le=100),
                offset: int = Query(0, ge=0),
                db: Session = Depends(get_db)):
    items, total = crud.list_filtered(db, search, "approved", limit, offset)
    return {"items": [crud.serialize(artist) for artist in items], "total": total}


@router.get("/band/artists/{artist_id}", response_model=schemas.BandArtistProfileResponse, tags=["Band Artists"])
