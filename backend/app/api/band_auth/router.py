...
@router.post("/band/auth/verify-email", tags=["Band Auth"])
def verify_email(payload: schemas.BandVerifyEmailRequest, db: Session = Depends(get_db)):
    account = crud.verify_email_token(db, payload.token)
    if not account:
...
@router.post("/band/auth/resend-verification", tags=["Band Auth"])
def resend_verification(payload: schemas.BandResendVerificationRequest, db: Session = Depends(get_db)):
    email_sent = crud.resend_verification(db, payload.email)
    message = (
        "Verification email resent successfully." if email_sent else
...
@router.put("/band/auth/admin/users/{user_id}/verify", response_model=schemas.BandAccountResponse, tags=["Band Auth Admin"])
def admin_verify_user(user_id: int, db: Session = Depends(get_db), _admin=Depends(require_band_admin)):
    account = crud.verify_account(db, user_id)
    if not account:
        raise HTTPException(status_code=404, detail="User not found")
    return account
