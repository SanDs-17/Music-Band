...
def register_user(
    client, email: str, name: str, role: str, password: str = STRONG_PASSWORD
):
...
class TestRegistration:
    def test_client_registration_assigns_client_role(self, client):
        ...
        assert any(
            r["name"] == "client" for r in roles
        ), f"Expected 'client' role, got {roles}"
    ...
    def test_artist_registration_assigns_artist_role(self, client):
        ...
        roles = data["data"]["roles"]
        assert any(
            r["name"] == "artist" for r in roles
        ), f"Expected 'artist' role, got {roles}"

    def test_venue_owner_registration_assigns_venue_owner_role(self, client):
        resp = register_user(
            client, "venue1@test.com", "Test Venue Owner", "venue_owner"
        )
        assert resp.status_code == 201, resp.json()
        data = resp.json()
        roles = data["data"]["roles"]
        assert any(
            r["name"] == "venue_owner" for r in roles
        ), f"Expected 'venue_owner' role, got {roles}"
    ...
    def test_admin_registration_blocked_on_public_endpoint(self, client):
        ...
        ), f"Admin self-registration should be blocked, got {resp.status_code}"
    ...
    def test_registration_with_email_failure_still_returns_201(self, client):
        ...
        with patch(
            "app.core.email.EmailService.send_verification_email",
            side_effect=Exception("SMTP unavailable"),
        ):
            resp = register_user(
                client, "emailfail@test.com", "No Email User", "client"
            )
        assert resp.status_code == 201, resp.json()
        assert resp.json()["success"] is True
        # Message should indicate email send failure, not success
        assert (
            "failed" in resp.json()["message"].lower()
            or "resend" in resp.json()["message"].lower()
        )
...
class TestPasswordStrength:
    ...
    def test_strong_password_accepted(self, client):
        resp = register_user(
            client, "strongpass@test.com", "Strong Pass User", "client", STRONG_PASSWORD
        )
        assert resp.status_code == 201, resp.json()
...
class TestLogin:
    def test_login_success_returns_tokens(self, client):
        ...
        resp = client.post(
            "/api/v1/auth/login",
            json={"email": "loginok@test.com", "password": STRONG_PASSWORD},
        )
    ...
    def test_login_wrong_password_returns_401(self, client):
        ...
        resp = client.post(
            "/api/v1/auth/login",
            json={"email": "loginbad@test.com", "password": "WrongPass@1"},
        )
    ...
    def test_login_nonexistent_email_returns_401(self, client):
        resp = client.post(
            "/api/v1/auth/login",
            json={"email": "ghost@test.com", "password": STRONG_PASSWORD},
        )
        assert resp.status_code == 401, resp.json()
    ...
    def test_login_response_has_standard_error_shape_on_failure(self, client):
        ...
        resp = client.post(
            "/api/v1/auth/login",
            json={"email": "nobody@test.com", "password": "BadPass@1"},
        )
        body = resp.json()
        assert "error" in body, f"Expected 'error' key in response, got: {body}"
        assert (
            "message" in body["error"]
        ), f"Expected 'message' in error, got: {body['error']}"
        assert (
            "detail" not in body
        ), "Raw FastAPI 'detail' key must not appear in error responses"
...
class TestEmailVerification:
    def _get_verify_token(self, client, email: str, role: str = "client") -> str:
        ...
        payload = pyjwt.decode(
            access_token, settings.effective_secret_key, algorithms=[settings.ALGORITHM]
        )
        user_id = payload["sub"]
    ...
    def test_invalid_token_returns_401(self, client):
        resp = client.post(
            "/api/v1/auth/verify-email", json={"token": "this.is.garbage"}
        )
        assert resp.status_code == 401, resp.json()
        assert resp.json()["success"] is False
...
class TestResendVerification:
    def test_resend_works_for_unverified_user(self, client, db_session):
        ...
        if user:
            ...
            db_session.commit()
        with patch(
            "app.core.email.EmailService.send_verification_email", return_value=True
        ):
            resp = client.post(
    ...
    def test_resend_cooldown_enforced(self, client):
        ...
        with patch(
            ...
            client.post(
                "/api/v1/auth/resend-verification", json={"email": "resend2@test.com"}
            )
    ...
    def test_resend_for_nonexistent_email_returns_404(self, client):
        resp = client.post(
            "/api/v1/auth/resend-verification", json={"email": "ghost_resend@test.com"}
        )
...
class TestVenueNumber:
    def test_venue_number_format(self):
        ...
        class MockDB:
            def execute(self, *a):
                raise Exception("No sequence in test")
    ...
    def test_venue_number_sequential_with_fallback(self):
        ...
        class MockDBFactory:
            ...
            def query(self, *a):
                return MockQueryIncrementing(self._count)
    ...
    def test_venue_number_starts_at_bcv_100001(self):
        ...
        assert (
            result == "BCV-100001"
        ), f"First venue number must be BCV-100001, got {result}"
...
class TestDecodeTokenExceptionShape:
    def test_expired_token_raises_unauthorized_exception(self):
        ...
        try:
            ...
            pytest.fail(
                "decode_token must not raise HTTPException — use UnauthorizedException instead"
            )
...
class TestChangePassword:
    ...
    def test_registration_creates_draft_artist_profile(self, client, db_session):
        ...
        ap = (
            db_session.query(ArtistProfile)
