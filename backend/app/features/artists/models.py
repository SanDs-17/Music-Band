...
class ArtistProfile(BaseModel):
    ...
    # Band Details
    band_type = Column(
        String(50), default="Solo", nullable=False
    )  # Solo, Duo, Trio, 4 Members, 5+ Members
    ...
    )  # E.g. {"own_speaker": true, "mic": true, etc.}

    # Availability schedules
    availability = Column(
        JSON, default=dict, nullable=False
    )  # E.g. {"weekly_schedule": ..., "holidays": ..., "blocked_dates": ...}
    ...
    videos = Column(
        JSON, default=list, nullable=False
    )  # ["youtube_url1", "vimeo_url2"]
    youtube_links = Column(
        JSON, default=list, nullable=False
    )  # Direct links entered in Step 6
    instagram_reels = Column(JSON, default=list, nullable=False)  # Instagram reel links
    pricing_details = Column(
        JSON, default=dict, nullable=False
    )  # { "hourly_rate": 5000, "travel_charge": 2000 }

    # Relationships
    user = relationship("User", backref="artist_profile")
    genres = relationship(
        "Category", secondary=artist_genres, backref="artists_in_genre"
    )
    languages = relationship(
        "Category", secondary=artist_languages, backref="artists_in_language"
    )
