...
class ArtistService:
    ...
    def register_artist(
        self, db: Session, data: ArtistRegisterRequest
    ) -> ArtistProfile:
        ...
        if not role:
            role = self.role_crud.create(
                db, obj_in={"name": "artist", "description": "Artist role"}
            )
        ...
        db.refresh(artist)
        logger.info(
            f"Artist/Band onboarding completed successfully for user {user.email}"
        )
        return artist

    def create_artist_profile_for_user(
        self, db: Session, user_id: str, data: ArtistProfileCreateRequest
    ) -> ArtistProfile:
        """
        ...
        db.refresh(artist)
        logger.info(
            f"Artist profile created for existing user {user.email} (id={user_id})"
        )
        return artist

    def update_verification_status(
        self, db: Session, artist_id: str, data: ArtistVerificationUpdate
    ) -> ArtistProfile:
        ...
        db.refresh(artist)
        logger.info(
            f"Artist profile {artist_id} verification updated to: {data.verification_status}"
        )
        return artist
    ...
    def suspend_artist(self, db: Session, artist_id: str) -> ArtistProfile:
        ...
        db.refresh(artist)
        logger.info(
            f"Suspended performer credentials session access: User ID {user.id}"
        )
        return artist
    ...
    def activate_artist(self, db: Session, artist_id: str) -> ArtistProfile:
        ...
        db.refresh(artist)
        logger.info(
            f"Activated performer credentials session access: User ID {user.id}"
        )
        return artist
    ...
    def get_or_create_draft_artist(self, db: Session, user_id: str) -> ArtistProfile:
        ...
        if not artist:
            ...
            artist = ArtistProfile(
                user_id=user_uuid, display_name=user_name, verification_status="pending"
            )
            db.add(artist)
            db.commit()
            db.refresh(artist)
    ...
    def update_profile(
        self, db: Session, user_id: str, data: ArtistProfileUpdate
    ) -> ArtistProfile:
        ...
        if data.genres is not None:
            ...
            for genre_name in data.genres:
                ...
                if not genre:
                    genre = Category(
                        name=genre_name, type="music_genre", is_active=True
                    )
                    db.add(genre)
                    db.flush()
                artist.genres.append(genre)
        ...
        db.refresh(artist)
        logger.info(
            f"Artist profile {artist.id} updated successfully by user {user_id}"
        )
        return artist
    ...
    def update_availability(self, db: Session, user_id: str, data: dict) -> dict:
        ...
        db.commit()
        logger.info(
            f"Artist profile {artist.id} updated availability schedule configurations."
        )
        return artist.availability

    def check_availability_conflict(
        self, db: Session, user_id: str, date_str: str, start_time: str, end_time: str
    ) -> tuple[bool, str | None]:
        ...
        try:
            work_start = datetime.strptime(
                day_config.get("start", "09:00"), "%H:%M"
            ).time()
        ...
        if req_start < work_start or req_end > work_end:
            return (
                True,
                f"Requested times fall outside performer working hours ({day_config.get('start', '09:00')} - {day_config.get('end', '22:00')}).",
            )
        ...
        try:
            break_start = datetime.strptime(
                break_config.get("start", "13:00"), "%H:%M"
            ).time()
            break_end = datetime.strptime(
                break_config.get("end", "14:00"), "%H:%M"
            ).time()
        except ValueError:
        ...
        if req_start < break_end and req_end > break_start:
            return (
                True,
                f"Requested times conflict with performer break hours ({break_config.get('start', '13:00')} - {break_config.get('end', '14:00')}).",
            )
        ...
        for event in mock_events:
            if event["date"] == date_str:
                ...
                if req_start < ev_end and req_end > ev_start:
                    return (
                        True,
                        "Performer has an event conflict at the requested times.",
                    )
    ...
    def get_pricing(self, db: Session, user_id: str) -> dict:
        ...
        return {
            ...
            "travel_radius": float(artist.travel_radius)
            if artist.travel_radius is not None
            else 0.0,
    ...
    def get_analytics(self, db: Session, user_id: str) -> dict:
        ...
        total_bookings = (
            db.query(Booking).filter(Booking.artist_profile_id == artist.id).count()
        )
        ...
        bookings = (
            db.query(Booking).filter(Booking.artist_profile_id == artist.id).all()
        )
        for b in bookings:
        ...
        for b in bookings:
            ...
            loc = (
                b.location.split(",")[-1].strip()
                if "," in b.location
                else b.location.strip()
            )
            if loc:
        ...
        reviews = (
            db.query(Review)
            .filter(Review.artist_profile_id == artist.id)
        ...
        for r in reviews:
            rating_trends.append(
                {"date": r.created_at.strftime("%Y-%m-%d"), "rating": float(r.rating)}
            )
