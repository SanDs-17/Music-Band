...
class VenueService:
    ...
    def register_venue(self, db: Session, data: VenueRegisterRequest) -> Venue:
        ...
        if not role:
            role = self.role_crud.create(
                db, obj_in={"name": "venue_owner", "description": "Venue Owner role"}
            )
        ...
        db.refresh(venue)
        logger.info(
            f"Successfully registered venue '{venue.name}' for user {user.email}"
        )
        return venue

    def create_venue_profile_for_user(self, db: Session, user_id: str, data) -> Venue:
        """
    ...
    def update_verification_status(
        self, db: Session, venue_id: str, data: VenueVerificationUpdate
    ) -> Venue:
        ...
        db.refresh(venue)
        logger.info(
            f"Venue listing {venue_id} verification updated to: {data.verification_status}"
        )
        return venue
    ...
    def suspend_venue(self, db: Session, venue_id: str) -> Venue:
        ...
        db.refresh(venue)
        logger.info(
            f"Suspended venue owner credentials session access: User ID {user.id}"
        )
        return venue
    ...
    def activate_venue(self, db: Session, venue_id: str) -> Venue:
        ...
        db.refresh(venue)
        logger.info(
            f"Activated venue owner credentials session access: User ID {user.id}"
        )
        return venue
    ...
    def get_dashboard_stats(self, db: Session, user_id: str) -> dict:
        ...
        return {
            ...
            "booking_stats": {"confirmed": 15, "pending": 3, "cancelled": 2},
            "occupancy_rate": 78.5,
            "calendar_overview": {
    ...
    def update_profile(
        self, db: Session, user_id: str, data: VenueProfileUpdateRequest
    ) -> Venue:
    ...
    def get_media(self, db: Session, user_id: str) -> dict:
        ...
        cover_image = (
            venue.metadata_fields.get("cover_image") if venue.metadata_fields else ""
        )
        youtube_links = (
            venue.metadata_fields.get("youtube_links") if venue.metadata_fields else []
        )
        virtual_tour = (
            venue.metadata_fields.get("virtual_tour") if venue.metadata_fields else ""
        )
        ...
        for item in raw_gallery:
            if isinstance(item, dict):
                if (
                    item.get("type") == "video"
                    or "video" in item.get("url", "").lower()
                    or item.get("url", "").endswith((".mp4", ".mov", ".avi", ".webm"))
                ):
                    videos_list.append(
                        {
                            "url": item.get("url"),
                ...
                if item.endswith((".mp4", ".mov", ".avi", ".webm")):
                    videos_list.append({"url": item, "category": "General"})
                else:
                    gallery_list.append(
                        {
                            "url": item,
    ...
    def update_media(
        self, db: Session, user_id: str, data: VenueMediaUpdateRequest
    ) -> dict:
        ...
        for vid in data.videos:
            db_gallery.append(
                {"url": vid.url, "category": vid.category, "type": "video"}
            )
    ...
    def update_availability(
        self, db: Session, user_id: str, data: VenueAvailabilityUpdateRequest
    ) -> dict:
        ...
        rules.update(
            {
                "weekly_schedule": {
                    ...
                    for day, val in data.weekly_schedule.items()
                },
                "blocked_dates": data.blocked_dates,
                "maintenance_days": data.maintenance_days,
                "public_holidays": data.public_holidays,
    ...
    def check_booking_conflict(
        ...
        if check_date_str in avail.get("blocked_dates", []):
            return {
                "conflict": True,
                "reason": f"Venue is explicitly blocked/reserved on {check_date_str}.",
            }
        ...
        if check_date_str in avail.get("maintenance_days", []):
            return {
                "conflict": True,
                "reason": f"Venue is closed for maintenance on {check_date_str}.",
            }
        ...
        if check_date_str in avail.get("public_holidays", []):
            return {
                "conflict": True,
                "reason": f"Venue is closed for public holiday on {check_date_str}.",
            }
        ...
        try:
            ...
            return {
                "conflict": True,
                "reason": "Invalid date or time formats. Use YYYY-MM-DD and HH:MM.",
            }
        ...
        for b in bookings:
            ...
            b_datetime_start = datetime.combine(req_date, b_start) - timedelta(
                hours=buffer_hours
            )
            b_datetime_end = datetime.combine(req_date, b_end) + timedelta(
                hours=buffer_hours
            )
            ...
            if max(b_datetime_start, req_datetime_start) < min(
                b_datetime_end, req_datetime_end
            ):
                return {
                    "conflict": True,
    ...
    def get_facilities(self, db: Session, user_id: str) -> dict:
        ...
        return {"facilities": facilities, "details": details}

    def update_facilities(
        self, db: Session, user_id: str, data: VenueFacilitiesUpdateRequest
    ) -> dict:
    ...
    def update_pricing(
        self, db: Session, user_id: str, data: VenuePricingUpdateRequest
    ) -> dict:
    ...
    def get_venue_analytics(self, db: Session, user_id: str) -> dict:
        ...
        bookings = (
            db.query(Booking)
            .filter(Booking.venue_id == venue.id, Booking.deleted_at.is_(None))
            .all()
        )
        ...
        upcoming_events_count = sum(
            1
            for b in bookings
            if b.event_date >= datetime.utcnow().date() and b.status == "accepted"
        )
        ...
        reviews = (
            db.query(Review)
            .filter(Review.venue_id == venue.id, Review.deleted_at.is_(None))
            .all()
        )
        average_rating = (
            sum(r.rating for r in reviews) / len(reviews) if reviews else 4.8
        )

        # 3. Monthly Revenue & Total Revenue from transactions
        total_revenue_query = (
            db.query(func.sum(WalletTx.amount))
            .filter(
        ...
        last_month_revenue = (
            float(last_month_revenue_query) if last_month_revenue_query else 0.0
        )
        ...
        this_month_accepted_bookings = sum(
            1
            for b in bookings
            if b.event_date.year == now.year
            and b.event_date.month == now.month
        ...
        if sum(p["value"] for p in revenue_chart) == 0:
            ...
            revenue_chart = [
                ...
                for m, v in zip(
                    mock_months,
                    [80000.0, 110000.0, 95000.0, 130000.0, 125000.0, 145000.0],
                )
            ]
            booking_chart = [
                {"month": m, "value": v}
                for m, v in zip(mock_months, [3.0, 4.0, 3.0, 5.0, 4.0, 5.0])
            ]
            occupancy_chart = [
                {"month": m, "value": v}
                for m, v in zip(mock_months, [30.0, 40.0, 30.0, 50.0, 40.0, 50.0])
            ]
        ...
        for b in bookings:
            ...
            if (
                "wedding" in name_lower
                or "marriage" in name_lower
                or "sangeet" in name_lower
            ):
                event_types["Weddings"] += 1
            elif (
                "corporate" in name_lower
                or "conference" in name_lower
                or "seminar" in name_lower
            ):
                event_types["Corporate Events"] += 1
            elif (
                "concert" in name_lower
                or "gig" in name_lower
                or "show" in name_lower
                or "live" in name_lower
            ):
        ...
        popular_event_types = [
            {"name": k, "value": float(v)} for k, v in event_types.items()
        ]
        ...
        top_clients = [
            ...
            for k, v in sorted(client_counts.items(), key=lambda x: x[1], reverse=True)[
                :5
            ]
        ]
        ...
        top_cities = [
            {
                "name": venue.city.name if venue.city else "Chennai",
                "value": float(total_bookings),
            }
        ]
        ...
        peak_seasons = [
            {"name": k, "value": float(v)}
            for k, v in sorted(month_seasons.items(), key=lambda x: x[1], reverse=True)[
                :4
            ]
        ]
    ...
    def resubmit_verification_documents(
        self, db: Session, user_id: str, data: VenueDocumentsResubmitRequest
    ) -> Venue:
    ...
    def update_venue_settings(
        self, db: Session, user_id: str, data: VenueSettingsUpdateRequest
    ) -> Venue:
