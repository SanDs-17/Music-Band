...
def create_booking_notification(
    ...
    try:
        ...
        elif event_type == "dispute_resolved" or (
            actor_role == "admin" and event_type == "override"
        ):
            # Notify Client
            notifications_to_send.append(
                {
        ...
        for notif in notifications_to_send:
            if not notification_preference_service.is_delivery_allowed(
                db, notif["user_id"], notif["type"]
            ):
                continue
            ...
            if notification_preference_service.is_realtime_allowed(
                db, notif["user_id"]
            ):
                from app.features.notifications.publisher import (
                    publish_notification,
                    serialize_notification,
                )

                publish_notification(
                    notif["user_id"], serialize_notification(created_notif)
                )

    except Exception as e:
        logger.error(
            f"Failed to generate booking notifications for event '{event_type}': {e}"
        )


def _write_failed_notif(
    db: Session,
    booking_id: UUID,
    actor_id: str,
    actor_role: str,
...
def create_failed_action_notification(
    ...
    if db is not None and is_testing:
        try:
            _write_failed_notif(
                db, booking_id, actor_id, actor_role, action, error_message
            )
        except Exception as e:
        ...
        try:
            _write_failed_notif(
                new_db, booking_id, actor_id, actor_role, action, error_message
            )
        except Exception as e:
            logger.error(f"Failed to record failed booking action notification: {e}")
        finally:
...
class NotificationService:
    ...
    def create_notification(
        ...
        if not notification_preference_service.is_delivery_allowed(
            db, recipient_user_id, notif_type
        ):
            return None
        ...
        if notification_preference_service.is_realtime_allowed(db, recipient_user_id):
            ...
            publish_notification(
                recipient_user_id, serialize_notification(created_notif)
            )

        return created_notif

    def get_notification(
        self, db: Session, notification_id: UUID, current_user_id: UUID, is_admin: bool
    ) -> Notification:
        ...
        if not is_admin and notification.user_id != current_user_id:
            ...
            raise ForbiddenException(
                "Access denied: Cannot access another user's notifications."
            )
    ...
    def list_notifications(
        ...
        if is_admin and target_user_id:
            ...
            raise ForbiddenException(
                "Access denied: Cannot list another user's notifications."
            )
    ...
    def get_unread_count(
        ...
        if is_admin and target_user_id:
            ...
            raise ForbiddenException(
                "Access denied: Cannot view another user's unread count."
            )
    ...
    def mark_read(
        ...
        notification = self.get_notification(
            db, notification_id, current_user_id, is_admin
        )
    ...
    def mark_all_read(
        ...
        if is_admin and target_user_id:
            ...
            raise ForbiddenException(
                "Access denied: Cannot modify another user's notifications."
            )
    ...
    def bulk_delete_notifications(
        ...
        if is_admin and target_user_id:
            ...
            raise ForbiddenException(
                "Access denied: Cannot delete another user's notifications."
            )
