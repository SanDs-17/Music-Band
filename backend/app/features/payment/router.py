...
async def create_payment_order(
    ...
    if booking.client_id != uuid.UUID(current_user_claims["sub"]):
        raise BadRequestException(
            "Access denied: You are not the client for this booking."
        )
    ...
    price = float(
        booking.counter_price if booking.counter_price else booking.proposed_price
    )
...
async def verify_payment(
    ...
    if booking.status not in ["accepted", "counter_offered", "pending"]:
        raise BadRequestException(
            f"Booking is already confirmed or cannot be paid: {booking.status}"
        )
